...
/**************************Initialize*********************************/
    ...
    public function initialize($kelas, $ruang, $waktu, $post, $prodi, $min_prosen_capacity){
        ...
        // echo '<pre>'; print_r($this->kelas); exit();
    ...
    public function create_individu(){
        ...
        $mapel_grup = array(); // untuk mengelompokan kelas berdasar matapelajarannya.
        $waktudistinct_grup = array(); // untuk menampung waktu_id hasil pengelompokan kelas berdasar mapelnya.
        ...
        foreach ($this->kromosom as $key => $value) {
            $arr_data = compact('timespace','individu','value', 'mapel_grup', 'waktudistinct_grup', 'grup_kemipaan','waktudistinct_grup_kemipaan', 'status_reset_individu');
    ...
    public function get_feasible_individu($arr_data){
        ...
        $ret_data = compact('timespace','individu','value', 'mapel_grup', 'waktudistinct_grup', 'grup_kemipaan', 'waktudistinct_grup_kemipaan', 'status_reset_individu');
        return $ret_data;
    }
    ...
    function get_random_local($individu_classprodi, $individu, $kelas, $timespace, $mapel_grup, $waktudistinct_grup, $period_waktu, $id_ts=null){
        /*
        membuat matriks ruang & waktu timespace_grup_waktu khusus untuk mapel yang sama,
        matriks ini waktunya sama, hanya ruang yang beda
        */
        $period_waktu = $kelas['period'];
        if (!empty($mapel_grup)) {
            if ($kelas['kls_jadwal_merata']==1) {
                $kode_kemipaan = $kelas['id_mpkur'].'-'.$period_waktu.'-'.$kelas['kls_jadwal_merata'].'-'.$kelas['kls_id_grup_jadwal'];
                foreach ($mapel_grup as $a => $mp) {
                    if ($mp == $kode_kemipaan) {
                ...
                foreach ($mapel_grup as $a => $mp) {
                    if ($mp == $kelas['id_mpkur'].'-'.$period_waktu) {
                        $timespace_grup_waktu = $this->get_id_timespace_for_sametime($timespace, $waktudistinct_grup[$a]);
                    }
                }
            }
    ...
    function getRandomTimespace($individu_classprodi, $individu, $value, $timespace, $period_waktu, $iteration, $timespace_grup_waktu=null, $id_ts=null ){
        ...
        if (!empty($value)) {
           foreach ($this->prodi as $p => $pd) {
               if ($pd['prodi_kode'] == $value['kd_prodi']) {
                   $id_ruang_pd = explode('|', $pd['ruang_id']);
                   break;
               }
           }
        }
        ...
        if ($id_ts==null) {
        	if ($value['praktikum'] == 1 && !empty($id_ruang_pd)) {
        		foreach ($timespace as $key => $item) {
        			foreach ($id_ruang_pd as $a => $b) {
            			if ($item['id_ruang']==$b && $item['jns_ruang']==1 && $item['status']!=1 && strtotime($item['waktu_jam_mulai'])>strtotime('14:15:00')) {
            				$timespace_prodi[] = array('id_ts_prodi' => $key);
                            break;
            			}
        			}
        		}
                ...
                foreach ($timespace as $key => $item) {
                    foreach ($id_ruang_pd as $a => $b) {
                        if ($item['id_ruang']==$b && $item['jns_ruang']==0 && $item['status']!=1 && $item['waktu_period']==$value['period']) {
                            $timespace_prodi[] = array('id_ts_prodi' => $key);
                            break;
                        }
                    }
            ...
            if (!empty($timespace_prodi)) {
                ...
                $id_timespace = $timespace_prodi[$id_time]['id_ts_prodi'];
            }else{
...
/**************************Count Fitness******************************/
    ...
    public function count_fitness_based_rule_kelasmapelsepaket_max_12_sks_sehari($individu){
        ...
        // echo '<pre>'; print_r($total_langgar); echo '</pre>';exit();
    ...
    public function count_fitness(){
        ...
        foreach ($this->populasi as $i => $individu) {
            ...
            $populasi[$i]['fitness_rule_2'] = $this->count_fitness_based_rule_kelasmapel_on_ruangblokprodi($individu);
            $populasi[$i]['fitness_rule_3'] = $this->count_fitness_based_rule_kelasmapelsepaket_max_12_sks_sehari($individu);
            $populasi[$i]['fitness_rule_4'] = $this->count_fitness_based_rule_kelas_filled_min_prosen_capacity($individu);
            $populasi[$i]['fitness_rule_5'] = $this->count_fitness_based_rule_kelas_guru_choose_their_time($individu);
...
/**************************Main Algorhitm*****************************/
    ...
    public function build_offspring_population_crossover_twopoint($parent_1, $parent_2, $point_random){
        ...
        foreach ($offspring as $key => $value) {
            ...
            $offspring[$key]['fitness_rule_2'] = $this->count_fitness_based_rule_kelasmapel_on_ruangblokprodi($value['offspring']);
            $offspring[$key]['fitness_rule_3'] = $this->count_fitness_based_rule_kelasmapelsepaket_max_12_sks_sehari($value['offspring']);
            $offspring[$key]['fitness_rule_4'] = $this->count_fitness_based_rule_kelas_filled_min_prosen_capacity($value['offspring']);
            $offspring[$key]['fitness_rule_5'] = $this->count_fitness_based_rule_kelas_guru_choose_their_time($value['offspring']);
    ...
    public function crossover(){
        ...
        if (empty($populasi_breeding_crossover_selected)) {
        	echo "Tidak ada nilai lebih kecil dari Probabilitas Crossover!";
            exit();
        }
